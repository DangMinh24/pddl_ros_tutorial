from pddl_parser.hddl_parser import (
    hddl_grammar_str
)
...
from information_extraction.htn_information_extraction import (
    extract_method,
    get_tasknetwork,
    get_subtask_defs,
    get_subtask_def,
...
if __name__ == "__main__":
    ...
    # print(result)
    # print()
    # print(" ".join(get_leaves(result)))

    methods = extract_method(result)
    # for method in methods:
    #     print (" ".join(get_leaves(method)))

    first_method = methods[4]
    networks = get_tasknetwork(first_method)
    print (networks)

    network = networks[0]
    partial_networks = get_subtask_defs(network)
    partial_network = partial_networks[0]

    list_of_nodes = get_subtask_def(partial_network)
    # TODO: ?Should we use additional general graph here for further clustering?
    general_graph = nx.DiGraph()

    fsm = nx.DiGraph()
    previous_symbol = None
    for node in list_of_nodes:
        task_symbols = get_task_symbol(node)
        task_symbol = task_symbols[0]
        task_symbol = "".join(get_leaves(task_symbol))
        fsm.add_node(task_symbol, data=task_symbol, shape="box")
        if previous_symbol is not None:
            fsm.add_edge(previous_symbol, task_symbol)
            previous_symbol = task_symbol
        else:
            previous_symbol = task_symbol

    general_graph = compose(general_graph, fsm)
    agraph_ = to_agraph(general_graph)

    # general_agraph_.add_edge("state",)
    agraph_.draw("finite_state_machine.png", prog="dot")
